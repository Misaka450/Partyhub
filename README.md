...
- **🧩 插件化模块解耦架构（维度一优化）**：
  ...
  - **前端插件插槽总线 (`window.PartyGames`)**：彻底终结 5,820 行“巨石文件”模式，将全部 21 款小游戏完整抽离为 `public/games/*.client.js` 独立模块，各游戏闭包作用域隔离、零命名冲突、即插即用，主干代码精简超 60%。
- **🛡️ 工业级安全与防作弊架构**：
  - 核心私密数据（卧底词、阿瓦隆角色、手牌、估数谜底）服务端单播隔离；
  - 观察类小游戏（多胞胎特征、瞬间数羊飞掠物）广播载荷剥离答案 ID 与目标标记，彻底根治控制台读包作弊；
...
### 脑力与观察挑战游戏 (9 款 · 统一收录于 `brainGames.client.js`)
...
| 16 | **🍪 谁不见了 / 偷吃怪 (Who Disappeared)** | 瞬时记忆 | 记住丰盛餐盘，在偷吃怪吃掉其中一件食物后迅速指出缺失项 |
...
- **Quality & Testing**:
  ...
  - 21 款小游戏引擎统一规范与防作弊读包安全契约套件 (`engine_contract.test.js`)；
