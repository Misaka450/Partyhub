...
- **⚡ 实时低延迟同步与性能防护**：基于 WebSocket (Socket.IO) 双向通信，单播状态唤醒；前端画笔 rAF 批量合帧；聊天与系统消息自动裁剪（上限保留 1000 条）彻底避免长会话 DOM 膨胀。
- **🛡️ 工业级安全与防作弊架构**：
  - 核心私密数据（卧底词、阿瓦隆角色、手牌、估数谜底）单播隔离；
...
| 9 | **🔢 密码破解大师 (Bulls & Cows)** | 逻辑推理 | 破解 4 位不重复神秘数字，即时计算位置与数字命中情况 (几A几B) |
| 10 | **🍕 极限切披萨 (Perfect Slice)** | 几何直觉 | 划线切割动态形状，仪表盘实时评定切片比例精准度 |
| 11 | **🧊 空间数方块 (Cube Count)** | 空间几何 | 3D 轴测等轴几何体生成，透视与盲区遮挡多维视角速算 |
| 12 | **🐑 瞬间数羊 (Flash Counter)** | 极速反应 | 动态视力大挑战，极短时间内统计动物数量、多寡对比或辨识幽灵动物 |
...
| 17 | **🎵 西蒙节拍记忆 (Simon Memory)** | 声光节拍 | 观察越来越长的四色声光节奏序列，按正序或倒序完整复现 |
...
- **Runtime**: Node.js
- **Backend**: Express, Socket.IO, Helmet, express-rate-limit, WebRTC 信令中继
...
- **Quality & Testing**:
  ...
  - 4-Tier 真机无头 CDP 体验级看门狗 (`ux_cdp_watchdog.js`：参数穿透/视窗尺寸/光影对比度/颁奖台完整性)；
